    ...
    <el-row>
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          ...
          <span v-if="is_edit === true">更新http服务</span>
        </div>
          ...
          <el-form ref="form" :model="form" label-width="140px">
            ...
            <el-form-item label="接入类型" class="is-required">
              <el-input v-model="form.rule" :disabled="is_edit===true" placeholder="路径格式：/user/,域名格式：www.test.com" class="input-with-select">
                <el-select slot="prepend" v-model="form.rule_type" placeholder="请选择" style="width:80px" :disabled="is_edit===true">
            ...
            <el-form-item>
              <el-button v-if="is_edit===true" type="primary" :disabled="submitButDisabled" @click="handleSubmit">立即修改</el-button>
              <el-button v-if="is_edit===false" type="primary" :disabled="submitButDisabled" @click="handleSubmit">立即提交</el-button>
            </el-form-item>
          </el-form>
        </div>
      </el-card>
  ...
  methods: {
    fetchData(id) {
      ...
      serviceDetail(query).then(response => {
        console.log(response.data.http_rule.rule)
